...
class listener implements EventSubscriberInterface
{
	...
	* @access public
	*/
	public function __construct(\phpbb\config\config $config, \phpbb\template\twig\twig $template, \phpbb\user $user, \phpbb\db\driver\driver_interface $db, $cache, \phpbb\auth\auth $auth)
	{
		$this->config	= $config;
	...
	*/
	public function add_stats_settings($event)
	{
		...
		if (($this->config['statsonindex_admin'] && $this->auth->acl_get('a_')) || (!$this->config['statsonindex_admin'] && $this->user->data['user_id'] != ANONYMOUS))
		{
			$display_output = true;
	...
	protected function obtain_activity_data()
	{
		if (($activity = $this->cache->get('_activity_mod')) === false)
		{
			...
			// cache this data for the same as online timespan, this improves performance
			$this->cache->put('_active_users', $activity, $this->config['load_online_time']);
		}
